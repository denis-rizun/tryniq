...
- **Query keys are arrays starting with the resource name, then identifiers**: `['meetings']`, `['transcript', id]`, `['chat', 'session', sessionId]`. Match the conventions already used in the codebase exactly so cache invalidation lines up.
...
- Cross-route UI state (AI drawer open, command palette open, export modal open, active chat session) lives in **one** Zustand store: `lib/store.ts` (`useUIStore`).
...
- Selectors: `useUIStore((s) => s.drawerOpen)` for fine-grained subscriptions when re-render cost matters; destructure (`const { setPaletteOpen, toggleDrawer } = useUIStore()`) when you need several fields together.
...
- Toasts (`sonner`) are reserved for cross-route lifecycle notifications (meeting finalized, export ready, chat session saved). Do not `toast()` from inside a component for routine UX feedback — render in-place state instead.
